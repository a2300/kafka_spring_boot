...
//        props.put("client.id", "");
        ...
        partitions.add(new TopicPartition("my-topic-other", 2)); // Adds an additional TopicPartition instance representing a different partition within the topic. Change as desired.
    ...
    private static void printSet(Set<TopicPartition> collection){
        if (collection.isEmpty()) {
            ...
            for (TopicPartition partition: collection){
                System.out.println(String.format("Partition: %s in Topic: %s", partition.partition(), partition.topic()));
            }
        }
    }
